...
def token_pair_features(tokens, feats, k=3):
    ...
    combine=[]
    for w in windows:
        combine.append(['_'.join(subset) for subset in combinations(w,2)])

    featArray=[]
    for pairList in combine:
        for item in pairList:
            featArray.append(item)

    for n in featArray:
        feats["token_pair="+n]+=1
...
def featurize(tokens, feature_fns):
    ...
    for f in feature_fns:
        f(tokens,feats)
    fList=list(sorted(feats.items(), key=lambda tup: tup[0]))
    return fList
    pass
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for tokens in tokens_list:
        features[index]= featurize(tokens,feature_fns)
        index+=1
    for f in features:
        for v in features[f]:
            if(cDoc[v[0]]==0):
    ...
    for f in cDoc:
        if(cDoc[f]>=min_freq):
            finaldict[f]=cDoc[f]
    finaldict= sorted(finaldict.items(),key=lambda x:(x[0]))
    if(vocab==None):
        count=0
        ...
        for f in features:
            for v in features[f]:
                if(cDoc[v[0]]>=min_freq):
                    row.append(f)
                    column.append(c[v[0]])
                    data.append(v[1])
        return  csr_matrix((data, (row, column)), dtype='int64'),c
    else:
        for f in features:
            for v in features[f]:
                if v[0] in vocab:
                    row.append(f)
                    column.append(vocab[v[0]])
                    data.append(v[1])
        return  csr_matrix((data, (row, column)), dtype='int64'),vocab
    pass
...
def cross_validation_accuracy(clf, X, labels, k):
    ...
    for train_ind, test_ind in cv:
        ...
        accuracies.append(accuracy_score(labels[test_ind], predicted))
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    for d in docs:
        t = tokenize(d, keep_internal_punct=True)
    ...
    for function in feature_functions:
        for punct in punct_vals:
            for freq in min_freqs:
                ...
                obser = {'punct':punct , 'features':function, 'min_freq':freq, 'accuracy':accuracy}
                combi_dict.append(obser)


    return sorted(combi_dict, key=lambda x:x['accuracy'], reverse=True)
    pass
...
def mean_accuracy_per_setting(results):
    ...
    for r in results:
        punctLabel.add(r['punct'])
        freqLabel.add(r['min_freq'])
        #print(type(r['features']))
        functLabel.add((r['features']))
    
    finalList=[]
    fLabel=list(functLabel)
    ...
    for p in punctLabel:
        ...
        for r in results:
            if r['punct']==p:
                acc.append(r['accuracy'])
        finalList.append((np.mean(acc),'punct='+(str(p))))
    ...
    for f in freqLabel:
        ...
        for r in results:
            if r['min_freq']==f:
                acc.append(r['accuracy'])
        finalList.append((np.mean(acc),'min_freq='+(str(f))))
    ...
    for fun in fLabel:
        ...
        for r in results:
            ...
            if r['features']==fun:
                acc.append(r['accuracy'])
            for f in fun:
                fname.append(f.__name__)
        fileName = ' '.join(str(e) for e in fname)
        finalList.append((np.mean(acc),'features='+(str(fileName))))
...
def fit_best_classifier(docs, labels, best_result):
    ...
    function = best_result['features']
    
    
    for d in docs:
        t = tokenize(d, keep_internal_punct=punct)
        tokens.append(t)
...
def top_coefs(clf, label, n, vocab):
    ...
    Flist = []
    if label == 0:
        index = sorted(range(len(clf.coef_[0])), key=lambda k: clf.coef_[0][k])[:n]
    else:
        index = sorted(range(len(clf.coef_[0])), key=lambda k: clf.coef_[0][k], reverse=True)[:n]

    for item in vocab.items():
        for i in index:
            if item[1] == i:
                if clf.coef_[0][i]<0:
                    Flist.append((item[0],-1.0*clf.coef_[0][i]))
                else:
                    Flist.append((item[0],1.0*clf.coef_[0][i]))


    return (sorted(Flist, key=lambda x:x[1], reverse=True))
    pass
...
def parse_test_data(best_result, vocab):
    ...
    X,vocab = vectorize(tokens, function, freq, vocab)
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for p in range(len(predicted)):
        dict1 = {}
        if predicted[p] != test_labels[p]:
            if predicted[p] == 0:
                dict1 ={'truth' : test_labels[p], 'predicted': predicted[p], 'probability': predict_probaval[p][0], 'test':test_docs[p]}
            else:
                dict1 ={'truth' : test_labels[p], 'predicted': predicted[p], 'probability': predict_probaval[p][1], 'test':test_docs[p]}
            finalList.append(dict1)
    finalList=sorted(finalList, key=lambda x: (-x['probability']))[:n]
    for f in finalList:
        print('truth=%d predicted=%d proba=%.6f'%(f['truth'],f['predicted'],f['probability']))
        print(f['test']+"\n")
    pass
